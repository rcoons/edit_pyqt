...
class TextEdit(QTextEdit):
    ...
    def __init__(self, filename="", parent=None):
        ...
        if self.filename == "":
            self.filename = "新建文本档-{0}".format(TextEdit.NextId)
            TextEdit.NextId += 1
    ...
    def closeEvent(self, event):
        if self.document().isModified():
            dlg = QMessageBox.question(self, "McOffice", "是否保存对'{0}'的修改?".format(self.windowName),
    ...
    def save(self):
        if self.filename.startswith("新建文本档"):
            self.filename = \
                QFileDialog.getSaveFileName(self, "保存文件", self.filename,
